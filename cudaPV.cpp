...
__global__ void imageprocessing(PvImage *lImages)
{
	        
    // Read width, height.
    uint32_t lWidths = lImages->GetWidth();
    uint32_t lHeights = lImages->GetHeight();
    unsigned char *imgs = lImages->GetDataPointer();

    GpuMat cvImage(lHeights, lWidths, CV_8UC1, imgs, GpuMat::AUTO_STEP); 
    ...
    GpuMat subImage=cvImage(crop_region); 
    ...
    while ( !PvKbHit() )
    {
        ...
        if ( lResult.IsOK() )
        {
            if ( lOperationResult.IsOK() )
            {
                ...
                PvImage *lImage
                cudaMalloc((void**)&lImage, sizeof(PvImage))
                lImage = lBuffer->GetImage();
		        imageprocessing << <64, 128>> >(lImage); 
                cudaFree(lImage); 
